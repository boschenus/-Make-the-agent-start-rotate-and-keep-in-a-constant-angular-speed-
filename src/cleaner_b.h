#ifndef __CLEANER_B_AGENT__H
#define __CLEANER_B_AGENT__H 
...
/**The Controller of blue Cleaner, will serach the junk and take it to the blue side*/
class CleanerBController : public Process, public AgentInterface {

    public:
    CleanerBController() : Process(), AgentInterface() {}
    ...
    bool get, stop_r, search, stuck, deliver, adjust, go;
    int junk_id, ignore_id;
};
class CleanerB : public Agent {
    public:
    CleanerB(json spec, World& world) : Agent(spec, world) {
        add_process(c);
    }
    private:
    CleanerBController c;
};

DECLARE_INTERFACE(CleanerB)
